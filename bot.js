...
// Ad links + per-ad delay (seconds), configurable via .env
...
function createBot(token) {
  ...
  bot.on("message", async (msg) => {
    ...
    try {
      // Show each ad link, one at a time, with a delay before moving on
      ...
      await sleep(1000); // brief 1-2s pause before delivering, as requested
